...
**You can follow a writer rather than a paper.** Your collections hold a page of every byline your feeds have signed, searchable and in your own alphabet, and singling one out gives you a square of everything they have written wherever it appeared. It is the one judgement that crosses publishers : a favourite source is one address, and a person moves between them. It stars nothing and changes nothing about the articles, and a writer you kept stays yours even after the last of their pieces has gone.

**Your sources are grouped by publisher, and there is nothing to file.** A paper with a feed per desk is one heading, worked out from the address it serves them at, so the list is organized the moment you subscribe and never goes stale. Every article says the publisher it came from and wears the one mark that publisher drew, rather than naming whichever desk it happened to arrive through. Call a publisher what you call it rather than what its address says, and make one a favourite when it is one of yours : a favourite source is not a starred article, and the two sit as separate squares in your collections. Change what a source is when it changes : its name, the site it belongs to, how often it is asked, the address it is served at, and whether that address is a secret. A feed that moves keeps its articles and everything you said about them, here and on your other devices, where it used to mean unsubscribing and starting again. Stop following a source, or a whole publisher, and everything that was only there because of it goes with it, on this device and in your iCloud, after Flong has said so and you have said yes.
...
**Sources you pay for stay readable.** A per-subscriber feed address, HTTP Basic or a token, kept in the keychain. Say, in the source's own editor, which parameters of its addresses carry your subscription, and Flong takes exactly those off any link that leaves the device, and nothing else : a parameter selects a feed or a filter as often as it names a subscriber, so it is never guessed at. For a site whose articles sit behind a wall, sign in on its own page and Flong fetches the rest as you : it never holds your password.
...
**It is set like a page, not like a control panel.** One column at a time, held to a readable measure, hairline rules, no cards and no boxes. Under the default theme the top of the front page takes the colours of the photograph the page leads on, and the colour is spent by the time that photograph comes into view ; open a story and its own page opens in the colour of the picture you pressed. Liquid Glass stays in the navigation layer, where Apple puts it, apart from the subject pills and the credit in the corner of a picture.
