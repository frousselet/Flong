...
- **Plain sentences in the interface** : a label, a section footer or a hint under a field is written in short, direct sentences a reader takes in at a glance. The literary register of the doc comments stays in the code and never reaches a user-facing string : no piled-up subordinate clauses, no enumeration of every rule the code applies, no clever phrasing. Say the thing that matters, then stop.
...
- **Commit messages are short** : a subject line, and a body only when the *why* is not obvious from the subject. When there is a body, keep it to a few lines of plain prose. No bold headings, no section per change, no restating the diff. The reasoning belongs in the code's doc comments, the documentation under `docs/`, and the pull request ; a commit message that repeats it is three copies to keep true.
- **English everywhere on GitHub**, without exception : pull request titles, descriptions **and comments**, issues, and review replies. The conversation being in French changes nothing about what is written there.
...
- **Bump the build number when a pull request is opened** : every time a PR is created, increment `CURRENT_PROJECT_VERSION` in `Flong.xcodeproj/project.pbxproj` (all configurations, they move together) and include the bump in the branch before opening the PR. `MARKETING_VERSION` is not touched : it moves only on a release. **The number in the project may already be published**, since a build is uploaded outside the repository and nothing here records which ones went out : the new value has to be greater than the last published build, so ask rather than assume the next one is free.
