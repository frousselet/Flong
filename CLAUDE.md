...
- **Bump the build number when a pull request is opened** : every time a PR is created, increment `CURRENT_PROJECT_VERSION` in `Flong.xcodeproj/project.pbxproj` by exactly one (all configurations, they move together) and include the bump in the branch before opening the PR. **Never ask which number to use** : it goes up one notch per PR, always. `MARKETING_VERSION` is not touched : it moves only on a release.
