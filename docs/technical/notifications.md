...
**And it is best effort, which is a consequence of what an edition is.** Everything here is posted with no trigger at all, which means *now*. An edition could be the exception : a page finished before its hour can have its notice handed to the system in advance and delivered on the hour with Flong not running, through a `UNCalendarNotificationTrigger`, where `earliestBeginDate` promises only that a background task will not begin sooner than the moment it is given. That needs the period to stop short of its own hour, and it does not : an edition runs over the period the reader defined, so it cannot be written before that period ends. The notice therefore arrives when the page comes into being, which is on the hour where a window is open or the system is generous, and later where it is not.

**And a name, so it can be replaced.** An edition's notice took a fresh identifier every time it was posted, so two passes that both noticed one page had come out stacked two banners for one paper. It is its boundary now, which every device works out the same way.

**Where the reader is is asked at delivery.** Nothing is said to somebody looking at the page it is about, and that used to be asked at the moment of posting. It is asked one step later now, by the only thing that runs when the system offers the banner : an edition arriving while Flong is open is filed without a banner and without a sound rather than dropped altogether, since Flong being open is not the same as the front page being read, and a reader three articles deep should find the paper in the centre when they come up.
