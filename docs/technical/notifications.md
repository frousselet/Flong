...
**The one notice whose words are already written.** Everything else here is a sentence assembled from names, which is why `Announcement` is a function from names to a sentence and why almost all of what can be wrong about a notification is decided there. An edition arrives carrying a short list the model wrote over the whole page, in the reader's own language, and that is the notice : the points joined by a middle dot as the headlines are joined everywhere else here, a point holding commas of its own. A banner gives the body two lines, so what does not fit is what the reader opens the page for.

The edition names itself in the one bold line a banner gives a title, which is where the dateline stands on the page and for the same reason : an edition has no name of its own, and `Édition du matin` is what a reader would call it. Writing anything of our own on top would be a third opinion about a page that already has one, and the edition's name goes on the line between as the attribution it is.
...
**It is lodged before its hour, and the system delivers it on the hour.** Everything else here is posted with no trigger at all, which means *now*, and that is right for news found a moment ago. An edition is different in the one way that matters : it is written before its hour, so its notice can be handed to the system ahead of time and arrive with Flong not running. Apple is explicit that the other route promises nothing, `earliestBeginDate` saying only that the system will not begin sooner than the moment it is given, so a notice posted by code that has to be running is a notice that arrives whenever a background grant happens to land. A `UNCalendarNotificationTrigger` with the zone pinned into its components is the one thing here that names a moment, and it fires at that instant wherever the reader has since flown.

**A page whose hour has gone is said at once, and that is not a second path.** The announcement carries the hour in one field ; the delivery compares it against now and answers a trigger or nothing, and nothing is what every other notice here has always passed. Same field, same builder, same posting.

**Which means the guard on the reader being here had to move.** Nothing is said to somebody looking at the page it is about, and while posting was delivering that was the right place to ask. The question is now put twenty minutes before the answer matters, and the watermark is stamped on the way past, so a reader with Flong open at twenty to eleven would have lost the eleven o'clock notice with no later pass able to put it back : the same failure the articles' own watermark was once fixed for. Where the reader is is asked at delivery instead, by the only thing that runs then. An edition arriving while Flong is open is filed rather than dropped : a banner and a sound over the page they are about is telling somebody something they are looking at, and Flong being open is not the same as the front page being read.

**And a name, so it can be taken back.** An edition's notice took a fresh identifier every time it was posted, which was harmless while nothing was ever pending. It is its boundary now, which every device works out the same way : an hour the reader retracts at half past ten unmakes the page waiting for it and withdraws the notice by that name, and a name is all that is left once the row has gone.
...
**One article, one notice, and never a condensed one.** It counted them and named three of the headlines : `5 new articles`, and the reader had to open Flong to learn what four of the five were. A count is the one thing they can work out for themselves from a stack of banners. The condensing was written for the stories, where it is right, and it never fitted this question : somebody who singles out a newsletter, a colleague or a person in the news is asking about each piece.

**So the watermark is a column on the article.** A clock works for one sentence about everything a pass brought : the pass reads what arrived since it last spoke, says one thing, and moves the mark. Said one at a time it cannot. The read is bounded at two hundred against absurdity, and the mark moved past everything the bound left behind, so the two hundred and first article was never announced and never would be. Nobody noticed because the notice was a count : two hundred was as good as two hundred and forty for a sentence saying `240 new articles`. `entry.announced_at` is the queue now, and nothing is lost and nothing is said twice by construction rather than by arithmetic.

It is stamped whether a notice was posted or not, exactly as the mark moved either way, and for the same reason : a reader looking at the page an article lands on has seen it, and being told tomorrow about what they read today is worse than not being told.

**Two things are written down as already told on the way in.** The first fetch of a feed nobody has ever fetched brings its whole back catalogue, and an import brings a whole account : both are a reader receiving a history they already had, and a notice apiece would be hundreds of interruptions about it. `FeedRefresh.store` takes a flag for it, set from `feed.last_success_at` being null and set by hand by the service import, so the rule sits in the one place articles are written from a feed's items and cannot be forgotten by a second caller.

**What is left of the old mark is one thing an article cannot answer**, which is whether this device has ever listened at all. The switch on a source travels between devices and the mark does not, so a device meeting a source already switched on has to tell `the reader asked here` from `the reader asked on the iPad, and what is waiting is that source's back catalogue`. Nothing at all is the second, and the queue is emptied without a word.

**A burst sounds once and stacks by what was asked about.** A reader who asked about every source may get eight banners from one pass. Eight of them are eight pieces of news and that is what they asked for ; eight sounds in a row is a device somebody puts face down, so the first of a burst sounds and the rest arrive at the system's `passive` level. And the thread is per source, writer or person rather than one for every article notice there is : the stack is what a reader actually reads when several arrive at once, and a morning's five publishers folded into one pile says nothing. Five from one paper is one stack, five papers are five.

**The notice carries the article's own identifier.** The system keys on it, so posting the same article twice replaces the notice rather than stacking a second copy. It was a fresh `UUID` every time, which was harmless while nothing could ever be posted twice and is not the shape to keep once a notice is about one thing for ever.
...
**Several of them are several notices**, which is the whole of the change : the count and the three headlines under it are gone, and each article stands on its own with its own picture and its own tap.
