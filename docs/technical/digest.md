...
**Eighteen words a point, and the bound is on the writing rather than on the drawing.** Points at three and four lines apiece were a wall of type before the first headline. What holds a point short is the model : it is asked for at most eighteen words, and nothing anywhere cuts a point that comes back longer.

Words and not characters, because the page has stopped counting lines. The old bound was a hundred characters and its argument went : a point stands on three lines, a line is filled with letters, so count letters. It was measured across the six hundred and eighty point measure, where three lines of body hold about two hundred characters ; the page it shipped to was a phone, where the same three lines hold about a hundred and ten, and a reader above the default type size met a sentence ending in an ellipsis, which is the one thing the bound existed to stop. A line count is a promise the writing cannot keep : it depends on the column, the face, the type size and the platform, none of which the model can be told and all of which the reader may change while the edition is on the screen. So the words bound the thought and the lines are left to the layout, which is the only thing that knows how wide the page is. A bound in characters was also quietly two bounds, since a word and its space run to about six characters in French and five and a half in English : the same rule gave a French reader less to be told than an English one.

Eighteen is the old bound's own arithmetic. A hundred characters was about sixteen or seventeen words all along, with a word of slack on top, so a good point of seventeen words is not thrown away for being one over. A page that cut a point to fit would be throwing away the end of a sentence, and one that set it smaller until it fitted would be whispering the news, so neither is done, on any of the three screens a point is drawn on.
...
**And the list stands on a pane of its own.** Three sentences of body type above ten stories of body type, told apart by a hairline and by nothing else, is a wall of grey a reader has to sort out for themselves : where does the edition stop and the news begin. The list is laid on a pane now, which says it in one move. It is the material and not a fill, and `docs/technical/interface.md` carries the argument for drawing the material here at all. One pane and not one per point, since three panes with three shadows at the head of a page is three objects where there is one thing being said.

What tells the points apart inside it is air, not a rule. A hairline apiece made this the one place on the front page where a rule ran inside an object : uniform type between hairlines in a rounded rectangle is the shape of a grouped table of settings, and the rule that used to sit under this pane was deleted for less.

**And all three are set alike.** They were ranked for a while, the first in the theme's display face and the two under it standing back, on the argument that the model is asked for what matters in the order it matters. The premise was wrong : three points are three things worth knowing equally, nothing scores them, nothing checks the order they come back in, and a page that set one of them larger would be inventing a hierarchy and then asking the reader to believe it. They are prose, so they take the standfirst's face rather than the headline's, which is where a theme gets out of the way ; asked for through the token rather than written as a bare `body`, so the pane stops being the one block of type on the page that asks nobody. All three carry a grade over the news under them, which is the distinction the pane is actually for : a standfirst has never been set the same as the paper, and a grade that lifts the whole list at once lifts no point above another. It is what the ground asks for too, since this is the one block of type in the application whose ground is different every morning : the material resolves against the colour a publisher's photograph gave the page, it frosts for a reader who asked for less transparency and goes stark for one who asked for more contrast, and one grade of ink holds on all of those where anything tuned to how much shows through holds on one.

**And one drawing, not three.** The same sentences were set out by hand at the head of the front page, again at the head of a back number, and again in the row that opens one, with two mark weights, two column gaps and three spacings between them, and only the archive row ranking anything. A reader who opened yesterday's edition from the calendar met a plainer copy of this morning and nothing on the page explained why. There is one drawing now, and what a caller chooses is how many points to show and what ground to lay them on.

**And it goes back rather than up as the page is scrolled.** Every other row leaves by the top edge, which is right for a story : it is one of forty and the next one takes its place. What the edition says is the page's own voice and there is one of it, so it is held back against the scroll at half speed, shrinks and is gone by the time the first headline reaches where it stood, and the news slides over it.

The fade and the shrink are eased rather than run straight off the distance travelled. Straight, the pane was at half its ink after a hundred points of scroll, which is while it is still the thing the reader is looking at, and something that begins to leave the instant the page is touched reads as something that was never quite there. Eased on `3t² - 2t³` it barely moves through the first third and is gone through the last. The drift is not eased with them : it is the reader's own finger, and a parallax running ahead of the scroll it is held back against would be a pane sliding on its own. The travel is scaled with the reader's type, since the pane grows with the type and a fixed travel faded the head out while most of it was still on the screen.

**And it stops for a reader who asked for less motion.** The fade stays, since what it says is that the head is behind the news now, which is information ; the drift and the shrink are the movement itself, and they stop. It is what the live dot does under the same setting, and the skeleton's band of light.
...
**And it is set open.** A point runs to two or three lines and the next one is a different subject : set solid, three of them ran together into a block the eye reads as one. The air inside a point is what says where it ends, and the air between two of them is three times it, since two points set as openly as the lines inside one are two points a reader cannot tell apart. The pane's inset is one number on all four sides, a shade wider than that gap : an outer margin narrower than the gap inside is a block that has slipped off its own paper. Both the air inside a point and the air between two of them follow the reader's type, since the ratio between them is what the reader reads and a ratio frozen at one size is lost at every other. The skeleton stands a bar and its air to the height of a line of type plus its leading, and it scales with the reader's type, or the page moves by the difference when the words land, which is the whole of what it exists to stop. Its bars are drawn a rung darker than every other skeleton in the application, because they are the only ones laid on the material rather than on paper, where the lightest fill there is was measured coming back at pure white.
...
It is set into the line rather than beside it. It had a column of its own, twenty points wide, and every wrapped line of a point came back to the far side of it : a label pinned next to a paragraph holds a gutter open down the whole pane, takes the width away from the words at the size where they have least to spare, and a point of three lines indented past a glyph reads as a quotation. It matters more now than it did, since a point is no longer held to three lines. A glyph that is part of a sentence has to break with the sentence, so the mark and the words are one run of text and not two views.

**A point that matches nothing wears the tag.** Half a mark on a row of marks reads worse than a neutral one, and a point about a story the filing never reached is an ordinary state rather than a fault. The skeleton draws no mark and holds no room for one : there is no column to hold open, and an inline mark is about one character at the head of the first line. What the skeleton owes the page is the height of the block, which the mark does not change.
