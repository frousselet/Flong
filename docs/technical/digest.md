...
**A paper goes to press before it comes out.** The period ran to the hour, so the model could only be asked once the hour had passed, so the notice could only be posted when a background pass happened to run. Apple is explicit that this is not a promise : `earliestBeginDate` says only that the system will not begin sooner than the moment it is given. Written twenty minutes early, the notice is lodged with the system and delivered on the hour with nothing of ours running, which is what `UNCalendarNotificationTrigger` is for.

Twenty minutes, and the reader pays for every one of them : the 23:00 paper closes at 22:40 and what arrives after that opens the next period. It is the shortest run that holds the writing, being the wait for a straggling headline and one further ask after it. Longer buys a wider window for a grant to land in and costs the reader news minute for minute, and no width is a promise, so buying probability against a scheduler that promises nothing is a bad trade paid for in news.

**Written is not out.** `published_at` still means the moment the model answered, set once and never cleared ; what decides whether a page may be *shown* is `Edition.out(by:)`, which is `published_at IS NOT NULL AND opened_at <= now`. There are two readers of that column and they go through one scope, because patching only the obvious one would have handed the reader their eleven o'clock page at twenty to eleven under a dateline saying otherwise.

**A device with nothing on the table does not press ahead.** A fresh install at a quarter to seven would otherwise write the seven o'clock page and then correctly hide it, leaving the reader on the skeleton for a quarter of an hour with a finished page in the store. Where no edition has come out, the boundary that has *gone* is minted and run to, exactly as this always did, and the presses start from the next one.

**A schedule that moves unmakes a page rather than relabelling it.** An hour retracted at half past ten, here or on another device, would leave a finished eleven o'clock page to arrive on the front at eleven under a banner the system delivers punctually for an hour that no longer exists. A page whose hour has not come has been seen by nobody, an edition being the one thing here that does not travel, so it is deleted and its notice taken back. That is the first thing in Flong to listen to `NSUbiquitousKeyValueStore.didChangeExternallyNotification`, and it has to be : `load()` reads the schedule on its own beat, which was enough while a page was made at its hour and never before it.
