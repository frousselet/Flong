...
**The page draws its own shape before it has one.** An edition is not published until the model has written the whole of it, so the front page stood empty for the seconds or minutes that takes and then arrived all at once, pushing everything below it down the screen : a reader who had started reading lost their place to a page they had not asked to change. It draws the list, the lead and one row behind it as bars now, through `redacted(reason: .placeholder)`, and the dateline is read off the schedule rather than off the edition so even that line is right from the moment the page opens.
...
**No mark in front of it either.** A story row carries one and there it says something : a story's line is the model's or its publisher's, and the mark is how a reader tells which. Nothing on an edition's head is ever anybody else's, an edition existing only where the model wrote the whole of it, so a mark here answers a question nobody can ask. It is still said to VoiceOver, where a statement costs no ink.
