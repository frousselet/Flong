...
A standfirst that comes back wrong twice is dropped rather than asked for a third time, and the article's own is used instead. It is at least a real one, written by somebody, and a third call comes out of the same budget as the subjects.
...
**A name the model coined first becomes the section, rather than blocking it.** The fold skipped whatever already existed, whoever had made it. On a store where the model had already named a story `Politique` or `Technologie` for itself, those two stayed the model's own, and since the model is only ever shown the settled subjects, it was never offered `Politique` again and nothing could ever be filed under it : two of the thirteen sections were missing from the vocabulary and held nothing, for good. They are promoted now. A section the reader wrote themselves stays theirs, being theirs to delete, which a standard one is not.
