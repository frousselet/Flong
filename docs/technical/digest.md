...
**A story says which model wrote its head.** The mark on the row is unchanged and stays one mark ; the sentence behind it names the author, because it used to promise that nothing had been sent anywhere and for a headline written elsewhere that promise is false. The name is copied onto the story rather than pointed at, so deleting the account does not rewrite what already happened.

**Who is asked is a decision per task.** There are four things a model does in this application : the headline and the line under it, the filing under subjects, the points over an edition, and the reading of a sentence typed into search. They used to be four callers of one namespace, so a model that had stopped answering had stopped answering all four and the interface could only say so in one sentence for the lot. Each of them now asks a desk for the hand that answers its own question, and what comes back carries the model, the patience that goes with it and the reading of its failures. The patience is per model rather than per process : three failures leave that one alone for ten minutes, and a second model is untouched by it.

**The shape is a value and not a type.** It was a `@Generable` struct per answer, which writes the shape into the type at compile time and serves five of the six answers asked for here. It cannot express the sixth : the subjects a story may be filed under are the reader's own list, read out of the store at the moment the question is put, so that call site already built a schema by hand and asked through it. There were two ways of asking, and the one that could express everything was used for one question out of six. `ResponseShape` is that one way, said once : four cases, rendered into the framework's own schema for the model on this device, and readable as a plain tree by anything else. What the model is told is unchanged, the text of every `@Guide` having moved into the shape word for word.
...
**It was a paragraph and had to stop being one.** Asked for two or three sentences over ten stories, the model wrote one clause per story and joined them with commas : seven items and eight lines of type, which is the shape a reader's eye slides off. Three points at most now, one thing to a line, in the order they matter. It was cut to five first, which was still most of a screen before the first headline ; three is what an edition is actually about, and the rest is on the page underneath. The numbering a model puts in front of its own list comes off, since it writes `1. ` or `- ` about half the time and the page draws its own marks, and the bound is kept in code as well as asked for : `maximumCount` guides a model and does not bind it.
...
**The dateline is decided at the first frame, and it has to be.** A navigation bar measures the height of its title area once : a subtitle that appears under the title later does not make it measure again. The line was read off the store, so it was empty for the moment it took the page to load, the bar was laid out nineteen points shorter than it belongs, and the page opened that far down and stayed there until the first scroll pushed the bar into measuring again. The reader saw the page settle downwards the moment they touched it. Every term of it is now known before anything is read : the hours come from the preferences, which are held rather than fetched, and whether there is a model to write an edition at all is a question about the device. It is empty only where no edition is coming, ever.
...
**The page draws its own shape before it has one.** An edition is not published until the model has written the whole of it, so the front page stood empty for the seconds or minutes that takes and then arrived all at once, pushing everything below it down the screen : a reader who had started reading lost their place to a page they had not asked to change. It draws the list on the pane it will stand on, with bars where the words go, and the lead and one row behind it, and the dateline is read off the schedule rather than off the edition so even that line is right from the moment the page opens.
...
**Eighteen words a point, and the bound is on the writing rather than on the drawing.** Points at three and four lines apiece were a wall of type before the first headline. What holds a point short is the model : it is asked for at most eighteen words, and nothing anywhere cuts a point that comes back longer, on the front page, in a back number or in the row that opens one.

Words and not characters, because a line count is a promise the writing cannot keep : it depends on the column, the face, the type size and the platform, none of which the model can be told and all of which the reader may change while the edition is on the screen. The old bound was a hundred characters, measured across the six hundred and eighty point measure where three lines of body hold about two hundred ; the page it shipped to was a phone, where the same three lines hold about a hundred and ten, so a reader above the default type size met a sentence ending in an ellipsis, which is the one thing the bound existed to stop. A bound in characters was also quietly two bounds, since a word and its space run to about six characters in French and five and a half in English : the same rule gave a French reader less to be told than an English one. Eighteen is the old bound's own arithmetic said in the unit that survives a change of column, with a word of slack on top. A page that cut a point to fit would be throwing away the end of a sentence, and one that set it smaller until it fitted would be whispering the news, so neither is done.

**And no two points say one thing.** A page led by a story big enough to be written about twice came back with `Une attaque russe a frappé le siège du SBU à Kiev` and, under it, the same sentence in other words : a third of an edition of three spent saying one thing. The points are compared by the words they are made of rather than by their letters, since a model rephrases rather than repeats, and two sharing two thirds of what either of them is about are one point. The model is told, and asked again ; where it insists, the second one is dropped and the page shows two points, since two points is still a list and one thing said twice is not.

**A headline is a sentence of the reader's own language.** The model wrote `Argentine restitue tableau volé par nazis`, `Pierre Gasly pole position` and `Park Bench abandonne Bordeaux Girondins acquisition` : English titling applied to French, with the determiners dropped, the nouns stacked in the English order and sometimes no verb at all. English newspapers write that way and French ones do not, so what a reader met was a page that reads as broken. Both voices say so now, in one sentence with the wrong version beside the right one.

**And it is one sentence rather than seven, which is the part worth remembering.** The rule was first written out at length : a line for the determiners, a line for the noun stacking, a line for the verb, a line for the full stop, with an example under each. Every one of them was true and the brief stopped working : the model went on writing good headlines and began returning no standfirst at all, which `TopicNamerLiveTests` caught against the real model on the first run. A small model has a budget of attention as well as of tokens, and seven lines about the headline, inserted between the headline's paragraph and the standfirst's, spent the standfirst's share of it. What is asked for has to be weighed against everything else being asked for in the same breath.

And a headline ends without a full stop, being a title rather than a sentence of the article. That one is taken off in code as well as asked for, since it is one character and checkable, and a rule that lives only in a prompt is a rule a small model keeps most of the time : one headline in a list came back with a stop where no other had one, and the row read as a mistake. A question mark, an exclamation and an ellipsis are left alone, being punctuation that means something.

**And a point reports something that happened.** The model wrote `La Russie et l'Ukraine sont en guerre` at the head of a page, which is true, was true yesterday, and spends one of the three lines an edition has on telling the reader nothing about today. A condition that already held is not news and is never a point : the instruction names that sentence and two others like it, tells the model to write what moved in it instead, and tells it to leave a story out rather than restate that a situation exists. Both voices carry the rule, since a page refused by the first is written by the second and a rule in one of the two is a rule that holds half the time. It is checked at test time, on both.

**And a point says what happened, not who said it.** The model kept the attribution clause an article opens with, so the first line of an edition read `The Ukrainian president says a Russian drone hit the SBU headquarters in Kyiv` where the news is that a drone hit it. It is told so plainly, with the two forms side by side, and told that a speaker is named only where the speaking is itself the news : a resignation, a denial, a decision announced.

**And the list stands on a pane of its own.** Three sentences of body type above ten stories of body type, told apart by a hairline and by nothing else, is a wall of grey a reader has to sort out for themselves : where does the edition stop and the news begin. The list is laid on a pane now, which says it in one move. A fill and not the material : this pane moves on every frame of every scroll and is blurred on the way out, and a material is resolved against what is behind it every time it moves. `docs/technical/interface.md` carries the argument for drawing the material here at all. One pane and not one per point, since three panes with three shadows at the head of a page is three objects where there is one thing being said ; inside it the points are separated by a hairline, set in from the marks so the rule runs under the words and not under the column of glyphs.

**And it goes back rather than up as the page is scrolled.** Every other row leaves by the top edge, which is right for a story : it is one of forty and the next one takes its place. What the edition says is the page's own voice and there is one of it, so it is held back against the scroll at half speed, shrinks, softens and is gone by the time the first headline reaches where it stood, and the news slides over it.

**And it stops being drawn once it has gone.** The pane is held back against the scroll, which is exactly what keeps it near the screen instead of letting the stack leave it behind : faded to nothing it was still a shape of glass, sampling what is behind it on every frame of the rest of the scroll for a reader who cannot see it. Past the fade it is `hidden`, which keeps the room it takes and stops the drawing. It is not blurred on the way out either : a blur over glass is two passes off screen for every frame of the first two hundred points of every scroll, and the shrinking and the fading say the same thing between them. None of this shows on a simulator, which draws the material cheaply ; a device draws the real thing.

The pane is rasterized once and the parallax moves the picture of it, with room left round it for the shadow to sit in : `drawingGroup` draws into a bitmap the size of what it is given, and a shadow falls outside that, so rasterized flush the pane came out with its shadow cut off at its own edge.

The travel is read off ``PageOffset``, which the page already writes for the wash behind it and which is nought at rest by construction. Measured instead off the pane's own position inside the scroll view, as the first version did, it is not nought at rest but the height of everything above it : the pane was pulled up under the subjects the moment the page opened, before anybody had scrolled anything. It lives in a modifier of its own so that what re-reads the offset on every frame of a scroll is that modifier and not the front page.
...
**Nothing invalidates a page, because a page is asked about once.** A story's brief is invalidated by the articles it was written from, and that was extended to the edition : an article joining any story on it made it a slightly different page, and the sentence over it a question worth putting again. It was the right rule for a page that lived while its news was still arriving, and it is the wrong one for a page about a period that has ended, where there is nothing underneath left to change. `edition.brief_members` is gone with the question it existed to re-open.

What it cost is worth writing down, because it is the whole argument for the change. The ten stories on a page are the ten most active in the store, so almost any arrival anywhere re-opened the question, and the job was offered a turn on every fifteen-second slice of every pass : twenty to fifty asks a day, one to three turns each, for a page that needs four. The work set is `published_at IS NULL` now, and there are exactly four of those a day.

**What an edition holds is a period, and the period is one expression.** `covers_from` is `max(opened_at - the three days, the hour of the newest published edition)`, and `open` refuses to mint a boundary that is not later than the last page that came out. That one line answers six things at once : a slot switched off never published, so the next period stretches back over it ; a boundary a sleeping device missed was closed rather than published, so its news folds forward instead of falling between two pages ; a store with nothing published takes the three days, which is the right first page after an import ; and a slot moved backwards, a flight west and a duplicated minute all mint nothing rather than a row with an inverted period that no screen could ever show.

It is stored rather than worked out again, and that is deliberate. The schedule travels through the key-value store, so the reader may move an hour or switch a slot off while a page is being made, and a period recomputed against a schedule that has since changed would claim a stretch of time nobody lived through.

**Membership is an article in the period, and never a story whose last article is in it.** A story that broke at ten and gained one more at ten past eleven belongs on the eleven o'clock page, and a `last_at` test drops it for having moved on. It is open at the bottom and closed at the top, so an article landing on a boundary belongs to exactly one page. And there is a second arm on the story's own key, which is a UUIDv7 minted where the grouping runs : a publisher that stamps this morning's item with last week's date, and a fetch that arrives long after the fact, reach the page of the period they were *grouped* in rather than falling out of every page there is.

**The ranking is the edition's own, and liveness is not a term in it.** The page ranks what is happening now first, which is the right question to ask at a moment and the wrong one to ask about a stretch of time that has ended : it would put the period's last hour above the rest of it. And what is weighed is the story's weight *in the period* rather than over its whole life, or a story that ran all week would lead every edition of the week on its own history.

**A page below two stories is left unstamped rather than declined.** Two is what still makes a list and one is a headline under a dateline. Nothing is written, so an article arriving at twenty past can still make the page ; and the ring in the corner says nothing about a stage that will never finish.

**The period runs to the hour, and the notice is best effort because of it.** This was the one real trade in the whole change, and it was tried the other way round first. A page finished *before* its hour can have its notice lodged with the system through a `UNCalendarNotificationTrigger` and delivered exactly on the hour with nothing of ours running, which is the only way an edition arrives punctually : `BGTaskRequest.earliestBeginDate` promises only that the system will not begin sooner than the moment it is given, so a page written at its hour is a page announced whenever a grant happens to land.

It was refused, and the reason is worth writing down. Closing the period twenty minutes early costs the reader the last twenty minutes of every period, and they are always the most recent twenty : the edition of eleven would say what had happened by twenty to, under a dateline saying eleven, next to a wire showing the difference. A dateline a reader can check and find wrong is worse than a banner that arrives late. An edition runs over the period the reader defined, and that settles it.

So the model is asked once the hour has passed, and the notice goes out when the page comes into being. On a window that is open, that is seconds : the foreground clock wakes at the hour as well as when a feed falls due, because an hour is a moment there is a page to make with nothing having arrived. On a phone in a pocket it is whenever the system grants the task, which may be an hour later, and the dateline still says the hour it is about.

**One scope, two readers.** `Edition.out(by:)` is where both the front page and the archive ask which editions have come out. There is nothing to keep apart now that written and out are the same thing, and it stays because a page shown by one reader and not by the other would be a back number nobody can reach.
...
The page was built at grouping time and never again, so it was filled before the model had written a word and, only stories the model has written about being eligible, it found none and stayed empty for the whole of its life. It is composed again on every pass for as long as it is unpublished, which costs one read and, where the ten have not moved, no write at all. What happens once is the asking, and it is held back until the period's own stories have been written or the wait for a straggler has gone by. Rebuilding was one answer to that ordering problem, and freezing a page from the first thirty seconds of a five-minute pass is another way of getting it wrong : the page has to be chosen after the writing and asked about once, which is two rules rather than one.
...
**Publishing is once, and the moment is never cleared.** There is nothing left that could clear it : a published page is not composed again, not written again and not asked about again, whatever arrives underneath it. And the page shows the newest edition that has **come out** rather than the newest one written, so last night's stands until this morning's hour, which is what a paper on a table does.

**The archive is the back numbers.** Held to the same three days the stories are, since an edition older than that is a page of headlines whose articles have gone, and never the page on the table : a quiet reader whose periods keep failing to fill two stories legitimately keeps one page for days, and the age would eventually take it and leave the front page blank. Each is what it was, frozen when it came out rather than when it closed, in `edition_story` rather than joined to the story table.

**What freezes is what the page said, and what moves is what the story is.** The front page joined its frozen rows against the live stories and then threw the frozen half away, so a headline the model rewrote in the afternoon changed on a page printed that morning, and a story that fell out of the sixty newest vanished from it altogether. But a page frozen whole is a page whose article count is wrong within the hour, and the count is not a claim about the page : it is a claim about the story, which is still moving and which the reader can still open. So the headline, the line under it, the picture, the credit for the picture and the order are the page's and never move again ; the article count, the rooms, the last arrival, the sparkline and the subjects are the world's and go on being read live. The credit freezes with the picture because a story takes both from one and the same article on purpose, and a frozen address beside a live credit is a caption naming the wrong paper. The subjects stay live because a rubric is a fact about the story, the reader may re-file it, and the marks beside the points are read from the same filing, so freezing one and not the other would put two answers on one page.

**The subject a point is about is frozen and its glyph is not.** The match was worked out on every read, from a live join, so the marks beside a back number drifted as the filing caught up behind it and every archive page changed its marks at once when a reader edited a subject. What each line was about is part of what the page was, and `edition.point_topics` says it. The glyph is not : it belongs to the subject, so a reader who changes it sees it change everywhere.

The frozen rows are kept even where the story behind one has gone. A `compactMap` there is a page that shrinks behind the reader's back the first time a purge takes one of its stories, which is the exact thing the frozen rows exist to prevent : the row is drawn from its frozen half alone, and the figures it has nothing to say about are not drawn at all rather than being drawn as noughts. A purge that took an article shrinks a story and can tidy it away altogether, and a page from last Tuesday that lost a row would be an archive nobody could trust. It is the same rule the library keeps against the stream, and the story identifier travels beside the frozen head with no key, so a row whose story is still there opens it and one whose story has gone is a headline and nothing more.
...
A colour per section is not a code. Fifty-two hues cannot be told apart, and a reader who has to learn which of four blues is `Immigration` is reading a legend rather than a page. Eight can be learnt without trying : a section belongs to a family of news and the family is what is printed. Violet for the news of the state, teal for money and work, the green of a leaf for the land, red for the body and the harm done to it, orange for ordinary life, a deeper green for science, magenta for culture, and no colour at all for `Société` and `Culture`, which are the two that sort nothing. `TopicFamily` holds them, and `StandardTopics.Section` carries a section's family beside its name and its mark, so the three cannot come apart.

**None of the eight is a blue.** Blue is what can be pressed : it is the accent the system hands `Défaut`, and `Solarized` states a violet-blue of its own in the same band. A subject printed in one is a word the reader tries to tap, which is the one confusion a code of colours cannot afford, so the band from cyan to indigo is left to the controls and `TopicColourTests` holds it closed. It costs two hues : the news of the state was `#1D6CA8` and science `#5B54C0`, and they are a violet and a green now. That is why two of the eight are greens, the land leaning towards the yellow and science towards the blue it may not reach ; the alternative was a warm hue at the contrast a rubric needs, which is a brown beside the brown of ordinary life.
...
**And the naming's slice is taken rather than left over.** The two of them ran to the end of the turn and the edition was named with whatever remained, on the argument that one call for a whole page is cheap enough to go last. It is cheap, and what remained was nothing : a night's stories all want a headline, every story that gained an article wants its own written again, and both halves of that were still going when the clock ran out. The naming was reached with a deadline already gone, so no call was made and the page was never asked about, on the one pass of the day it mattered. That is the morning the reader reported : a phone the system did wake at seven, five minutes of headlines, and last night's paper still on the screen at ten. The writing and the filing stop a slice short of the end now, and the naming works to a slice of its own with a floor under it, since a deadline is read before a call and not during one : a headline begun a moment before the reservation would otherwise carry the whole of it away.
