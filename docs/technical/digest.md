...
What tells the points apart inside it is air and a step in the type, not a rule. A hairline apiece made this the one place on the front page where a rule ran inside an object : uniform type between hairlines in a rounded rectangle is the shape of a grouped table of settings, and the rule that used to sit under this pane was deleted for less. The three sentences are ranked instead, because the model ranked them : it is asked for what matters in the order it matters, and the page then set all three at one size, so an ordering that was asked for, written and checked was thrown away at the last step. The first point takes the theme's own face a step under the news it stands over ; the two under it are prose in the standfirst's face, a step back in the ink.
