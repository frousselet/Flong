...
**And the list stands on a pane of its own.** Three sentences of body type above ten stories of body type, told apart by a hairline and by nothing else, is a wall of grey a reader has to sort out for themselves : where does the edition stop and the news begin. The list is laid on a pane of glass now, which says it in one move and lets the page go on showing through. `docs/technical/interface.md` carries the argument for drawing the material here at all. One pane and not one per point, since three panes with three shadows at the head of a page is three objects where there is one thing being said ; inside it the points are separated by a hairline, set in from the marks so the rule runs under the words and not under the column of glyphs.
