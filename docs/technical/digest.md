...
**And it is set open.** A point runs to two or three lines and the next one is a different subject : set solid, three of them ran together into a block the eye reads as one. The air inside a point is what says where it ends, and the air between two of them is three times it, since two points set as openly as the lines inside one are two points a reader cannot tell apart. The pane's inset is one number on all four sides, a shade wider than that gap : an outer margin narrower than the gap inside is a block that has slipped off its own paper. The skeleton stands a bar and its air to the height of a line of type plus its leading, and it scales with the reader's type, or the page moves by the difference when the words land, which is the whole of what it exists to stop. It ranks its blocks too, since the pane does : three blocks of one height promise a flat list where the page is about to draw a lead.
