...
**The two favourites stand next to each other on purpose.** They are different judgements, and a page that put them at opposite ends would leave the reader to work out that they are not the same one : `Articles favoris` is what they starred, one piece at a time, and `Sources favorites` is everything a publisher they singled out has ever served. One square holds four articles and the other holds a morning's worth, which is the distinction saying itself. The first was called `Favoris` while it was alone on the page and could not be mistaken for anything ; the moment the second arrived, it had to say which favourites it meant.
...
**And the page is put back at its top when the gesture is over.** The space the control held is not always given back : the page stays pushed down by exactly its height, with the large title still open, until anything at all touches the screen and forces a layout. That fault is why the gesture was taken out of the application once already ; four attempts at its cause missed, and none of them could be reproduced on a simulator, where a synthesised drag never engages the control at all.

So this does not try to diagnose it. It asks for the one outcome the reader wants, the page back at its top, and asking moves the scroll view whether or not it had reclaimed the inset on its own. Whether the reader is near the top is kept as a flag rather than as the offset itself : the offset changes every frame of every scroll, so holding it rebuilt the whole page continuously while a thumb was on it, and rebuilt it hardest during the one gesture that must not be disturbed.

A beat first, so it is not fighting the control on the way out, and only while the reader is still near the top : a pull starts there, but a refresh takes seconds and they may have scrolled off to read something, and hauling them back would be the application taking the page away from them.
...
**One bar for the whole pass, and the words above it change.** Every stage carried its own count at first, so one pass ran a bar from nothing to full five times over. A reader doing one thing and waiting for one answer does not read that as progress ; they read it as an application that keeps starting over. A pass declares what it is made of before it begins and each stage owns a share of the one bar, measured within that share by the queue it is working through.
...
**No figures beside the words.** A count is a second measure of the same thing, disagreeing with the first : the rule is the whole pass and a figure can only ever be the stage, so `9 sur 112` sat under a bar four fifths of the way along and the reader had to decide which of the two to believe. The bar says how far ; the words say what.
...
**And it is not clipped to its own height.** Glass casts a soft shadow, and the rectangular clip that kept the band from spilling while it grew cut that shadow off where it was still dark : what the reader saw was a grey oblong with hard edges sitting behind a capsule with round ones. Unclipped, the capsule spills a little during the third of a second it is growing or shrinking, which the fade over exactly that third of a second covers, and it spills downwards into the rubric's own top rhythm rather than upwards into the subjects.
...
**Who published it is named on glass, between the two.** The bar carried the name as plain type and could not keep it : the page runs a photograph under the controls, and type laid on somebody's photograph is unreadable on half the photographs there are. A pill takes the colour of what is under it and stays legible over any of them, with the publisher's mark in front of the name as everywhere else, since a mark is recognized before a name is read. The bar gives its own items glass, so the pill hides that : two panes stacked on one another read as a smudge rather than as either of them.
