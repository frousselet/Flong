...
**Nothing rebuilds the page under the gesture.** A refresh the reader asked for writes throughout, so the observation ticks repeatedly while they are still holding the page down, and every tick used to replace the content : `refreshAll` reads everything back itself when it is done, which is the one reload that gesture needs. The observation stands aside while the refresh runs and drops the tick that outlives it, since that tick is the refresh's own echo and everything it would show has already been shown. The digest is also only assigned when it differs, `@Observable` notifying on the assignment rather than on the value, so writing back an identical page no longer rebuilds a list somebody is scrolled into.
