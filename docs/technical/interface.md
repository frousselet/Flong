...
**Each date is a pill too, under the glyph that says which date it is.** One clock for when it was published and the same clock turned back on itself for when it was changed : the pair has to read as *this time* and *that time again*, and two symbols from different families would be two things to learn instead of one. The word that used to say which is gone, since the glyph says it.
