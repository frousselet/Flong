...
Three of them, stacked on their own lines rather than in a row. A subject is a phrase, phrases are not the same length, and a row of them is a row that scrolls sideways : a suggestion the reader has to go looking for is a suggestion that did not suggest anything. They wear Liquid Glass, and the result count wears it too, in a capsule of its own : the page scrolls under the bottom safe area, and a count with headlines passing sharply through it is not a count anybody can read.
