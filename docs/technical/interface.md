...
The digest is also only assigned when it differs, `@Observable` notifying on the assignment rather than on the value, so writing back an identical page no longer rebuilds a list somebody is scrolled into.
