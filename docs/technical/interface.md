...
So this does not try to diagnose it. It asks for the one outcome the reader wants, the page back at its top, and asking moves the scroll view whether or not it had reclaimed the inset on its own. A beat first, so it is not fighting the control on the way out, and only while the reader is still near the top : a pull starts there, but a refresh takes seconds and they may have scrolled off to read something, and hauling them back would be the application taking the page away from them.
