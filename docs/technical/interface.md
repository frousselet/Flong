...
**The page is in three parts, and their order is the argument.** What the reader marked comes first and wears no heading, a heading over two squares everybody has being a heading that says nothing : favourites is what they starred, notes is what they wrote on. Then what they made themselves, under their own name. Then the months, which fall out of when a copy was kept and cost nobody any tidying.

**The middle band is the albums.** Its heading is drawn whether or not there is anything under it, unlike the others : a reader with no collections is exactly the reader who has to be shown where they are made. The square that makes one sits at the end of the band rather than in a toolbar, the way an album is added in Photos, since the place a thing is made is the place the things of that kind already are.

A collection is a tag, which was the plan before it was a need : section 4 says a tag applies to an article, a feed or a library item, and `tag` and `tag_binding` have been in the schema since v1. Everything lives under a `collection/` root, so a tag and a collection of the same name stay two things, and a name carrying the separator is refused rather than allowed to invent a level. Renaming keeps what is in it ; throwing one away keeps the articles, a collection being a way of looking at what was kept and not a place it is stored.
