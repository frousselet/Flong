...
**The edition is a third pass, and it asks for the least of the three.** A page made at an hour has to arrive at that hour, and the full pass cannot promise it : it wants the mains, comes round every six hours and jitters by three quarters of an hour on top, so a phone that is not left on charge would have had no morning paper. This one asks for a network and not for the mains, at the boundary itself, and fetches nothing at all : the collection has already brought the articles in. What the grant does is write what the period brought, file it, close the period and ask the model once.

**It is asked for at the hour and the system reads that as a floor.** `earliestBeginDate` promises only that the task will not begin sooner than the date given, never that it will begin then, so a page asked for at eleven is a page at eleven at best and an hour or two later often enough. Closing the period early would fix it, since a page finished before its hour can have its notice lodged with the system and delivered punctually with nothing of ours running ; it is refused, because an edition is about the period that ends at its hour and stopping short of that hour is a paper that lies about its own dateline. So the notice is best effort, and what actually makes an edition arrive is the foreground.

Being idempotent is what makes it safe, and it now is. An edition already made and already named is one query and no work : it was not, every turn choosing the ten again and writing them to two tables the store watcher follows, so every pass cost a window reload for a page nobody had touched. The same pass runs at the press, whenever a window opens, and after every catch-up, and only the first of those does anything.

**One wake per boundary, and it was being given away.** The grant for the hour that has just gone is the whole of what a sleeping phone gets for that hour, and the handler ended by asking for the *next* boundary whatever had come of this one. A morning grant that came down at two minutes past seven, inside the twenty minutes a page waits for its own stories to be written, found nothing ready and asked for noon : the morning edition was never written at all, and the reader opened Flong at ten to last night's paper. The re-arm is bounded now. Where a page is composed, unwritten, not declined and still inside its wait, the moment that wait ends is asked for instead of the next boundary. It happens once per boundary by construction rather than by a flag : what is asked for is at least the end of the wait, so a page still unwritten after that was not being held by the wait, and the next call finds nothing to ask for and falls through to the hour.

Beyond that, no second wake is asked for. `BGTaskScheduler` holds one pending request per identifier, so a preparation grant would have to be replaced by the hour's from inside its own handler, immediately after the application has just spent a grant, which is how a system that has just served this app defers the next one : eight requested wakes a day, on the least favoured class, to make the paper arrive later. The work rides the beats that already exist instead, which is the half-hourly refresh, every foreground return, the five-minute clock, the full pass on the mains and the hourly activity on a Mac. The clock also wakes at the hour itself, since that is a moment there is a page to make with nothing having arrived. That matters more than the task does : section 25 says background time is a bonus and the foreground is the mechanism, and an edition that only arrived when `BGTaskScheduler` felt like it would be an edition most readers never saw.
...
| `BGProcessingTask` | writes the period, then makes and names the edition, a network and no mains | seconds of work, at the hour |
