...
**The question is asked against real addresses.** `SourceEditor` lists what the feed's address and its recent articles' addresses actually carry, each with its value masked : a list of parameter names in the abstract is a list of things nobody recognizes, and printing the values would make the screen a strange place to keep a secret. It is offered for every feed and not only for the ones that look authenticated, since a plain feed with a token on every link inside it is exactly the case a reader would never think to look for.
