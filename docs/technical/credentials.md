...
**A passkey cannot be used here, and the reader is told so.** Passkeys are Safari's : `WKWebView` does WebAuthn only for an application holding the web browser entitlement, which a feed reader will never be granted. A site that offers one does not fail cleanly either, since it is the identity provider that fails rather than the page : Google shows `Un probleme est survenu` and advice about Bluetooth and nearby devices, which describes the cross-device flow it fell back to and says nothing at all about where the reader is. Every site offering a passkey offers something beside it, so the message shown when a sign-in leaves no session names the cause and the way round it. Nothing here can be fixed in the application ; what can be fixed is a reader left guessing.
