...
**And an article reaching the store from another of the reader's own devices is keyed exactly as one from a publisher is.** It was not : the archive path wrote no key at all, so those rows were exempt from all of this for ever, and it stamped them as having arrived at the moment they were read rather than at the moment the sending device received them, which made a fortnight of somebody else's backlog look like breaking news to every notice that asks what arrived since a moment. The rule lives in `ArticleKey.original(of:in:)`, in one place, because two copies of it are two chances to disagree about what a duplicate is.
