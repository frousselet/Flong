...
**A third half, for the one person the other two cannot name.** Somebody invited who has not accepted has no identity for CloudKit to resolve : no name components, no picture, and nothing but the address the invitation went to. They have written no card either, since they have not joined. Drawn as it comes that is `33695754884` with a `3` for an initial, against somebody the reader knows perfectly well, because the number is in their address book. So that is where it is asked, and what is found goes in columns of its own : it is what *this* reader calls that person, it is never written into anybody's card, it never travels, and another participant sees their own address book or nothing. It is looked up only for a participant the share would not name, so it never stands in front of somebody's own name for themselves. The question is put at the moment a collection actually holds somebody who cannot be named, never at launch and never on the chance ; refusing costs the name and leaves the address. iOS's limited access is a full answer here, since nothing wants the book, it wants the person.
