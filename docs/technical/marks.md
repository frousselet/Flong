...
- **One rebuild at a time, whoever asked.** `beginBatch()` on an index that already has a batch open throws an Objective-C exception, which no Swift `catch` can stop and which takes the process with it : the same reason the index is a named one rather than the shared one, and the same absence of anywhere to put a `catch`. Rebuilds overlap easily, since the window asks for one when it opens, a catch-up that brought articles asks for another, and singling out a source, a writer or a newsmaker asks for one apiece. Every one of those is an `await` on the main actor, `SpotlightIndex` is a value and the `CSSearchableIndex` behind it is not, so nothing was holding anybody back. `IndexBatches` queues them, and the decision in `rebuildIfNeeded()` is taken **inside** the queue rather than before it, so a second caller reads the state the first left and says no rather than writing the whole index again over one just written. It was found by a test run dying rather than by a reader, which is luck and not design.
