...
| `BGProcessingTask` | the edition, at the press of the hour it comes out, with `requiresNetworkConnectivity` and **not** `requiresExternalPower` |
...
**And a paper goes to press before it comes out.** An edition is written twenty minutes before its hour, which is what lets its notice be lodged with the system and delivered on the hour with Flong not running at all : `BGTaskRequest.earliestBeginDate` promises only that the system will not begin sooner than the moment it is given, so a page written at its hour is a page announced whenever a background grant happens to land. What it costs is the last twenty minutes of every period, which are not lost : they open the period of the paper that follows, both bounds being one function of one moment, so nothing falls between two editions and nothing is counted twice.
