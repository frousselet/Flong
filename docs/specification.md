...
- **The past editions are an archive**, held to the same three days the stories are, each as it was, frozen when it came out rather than when it closed. What is the page's own never moves again : the headlines, the lines under them, the picture, the credit for the picture, the order, and the mark each point wears. What is the world's goes on moving : the number of articles, the rooms talking about it and how long ago the last one came, which are facts about the story and not about the page. A back number is a whole page and not a longer list.
