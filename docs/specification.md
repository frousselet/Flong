...
- **Every edition carries a headline and a few lines of its own, written by the model over the whole page**, in the reader's language, and marked as the model's like everything else it writes. That is a different question from the one a story is asked : a story is one event in a few words, an edition is what the ten add up to.
