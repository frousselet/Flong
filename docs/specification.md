...
**Amended : the digest is an edition, made at an hour, of ten stories.** The page above is rebuilt on every fetch, from the sixty newest stories of three days, in an order that moves as articles arrive. So it was never twice the same page, there was no such thing as having read it, and a reader who looked at nine and again at ten saw it shift under them for reasons they could not see. It is made four times a day now, at hours the reader sets : morning, midday, evening and night by default, each of them movable and each of them switchable off.

- **Ten stories, and the rest is the wire.** Ten is what a person reads over a coffee. What did not fit is not hidden and never was : the section beside the digest holds everything that arrives, and the next edition may well lead on it.
- **Every edition carries a short list of its own, written by the model over the whole page**, in the reader's language : five points at most, one thing to a line, in the order they matter. That is a different question from the one a story is asked, a story being one event said in a few words. It was two or three sentences and the model wrote a clause per story joined by commas, which is a paragraph and not a front page.
...
- **An edition is known by the moment it opened**, not by a key one device minted, so two devices working from one schedule build one edition for one morning rather than two. Nothing about an edition travels : each device works out the same ten from the same articles and names them with its own model.
- **The past editions are an archive**, held to the same three days the stories are, each as it was : the name it was given and the ten headlines it led on, frozen when it closed. A back number is a whole page and not a longer list.
- **Narrowing to a subject is a question about everything**, so a pill still reads the whole of the three days. The cap belongs to the front page, which is what an edition is.
