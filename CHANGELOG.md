...
- The reading interface, on iPhone, iPad and Mac : one column at a time under a system tab bar of four sections, the sources reached from a button in the leading corner of every section rather than from a tab of their own, a sidebar on Mac, each section keeping its own stack, a stream of everything as it arrives broken by day beside the digest, under a pinned row of bars on glass counting a calendar month, exactly the days it has, the days still to come greyed, which follows whichever day the reader has scrolled to, carrying no count of what is unread, a read article marked by a tick at the end of its headline rather than dimmed, with an isolated web view following the system appearance, reading and starring, marking a view read, adding a feed from the address of a site, and refreshing on returning to the foreground or on a pull.
...
- Collections : starring or annotating an article copies it whole, so it survives the purge of the stream and the disappearance of its source, shown as a grid of squares with what the reader marked at the head of it and the months of keeping under it, and what is kept is handed to Spotlight and found in the system search.
- Synchronization between a reader's devices through their own private CloudKit database, told of another device's changes by a silent push, carrying subscriptions, kept articles and read states compacted into one record per month, keeping what the server says about each record so a second save is not refused, carrying the whole stream as one record per feed and per day rather than one per article, with nothing purged unless the reader asks for their disk back, and iCloud's refusals surfaced rather than retried blindly.
...
- A reader's menu in the same corner of every section, wearing the reader's own face : an optional name and picture kept in their own iCloud with nowhere to be sent, every subject there is with what was said about each, including the ones that have fallen off the page, a way to add and remove subjects of their own, the sites they are signed in to, and the command to write the digest again.
