...
- Sessions on sites the reader subscribes to : signing in happens on the site's own page in a web view, only the session it leaves is kept, it is sent to that site and no other, and a session the site has ended says so rather than quietly serving teasers.
- Full-text extraction : an article opens on what its feed sent, and asking for the whole one fetches the page behind it once, pulls the article out of the navigation, sidebars, share bars and comments around it, and keeps it beside the feed's version.
